...
# nameslist = namesinlist(target_list)
# Adds Names from Argument and puts them into namelist, returns list of names from cardlist
...
# c = getcard(this_card, supply, player.hand, "your hand", ["coin"])
'''target_list=None so that it does not save target_list everytime the function is called.
set target_list=None everytime the function is called'''
def getcard(name, supply, target_list=None, target_name="the supply anymore",
            categories=['action', 'coin', 'curse', 'victory'], upto=100):
    if not name in supply: # name of card
        print(name + " is not in this game.")
        return None
    if not target_list: # Always true since target_list is set to None evertime this function is called.
        target_list = supply[name]   # checks supply dictionary with key name into target_list. returns list of cards
    nameslist = namesinlist(target_list)  # returns list of names from target_list
    if not name in nameslist:
        print("There is no " + name + " in " + target_name)
        return None
    i = nameslist.index(name) # index where name is located in nameslist
    c = target_list[i]  #gets card at index i
    if c.category not in categories:
        catstring = categories[0]
        for i in categories[1:]: # starts from 1
            catstring = catstring + " or " + categories[i]
        print(name + " is not a " + catstring + " card.") # name is not a action or categories[i] card.
        return None
    if c.cost > upto:
        print(name + " costs " + str(c.cost)) # prints cardname costs (price) converts c.cost to a string.
        return None
    return c        # returns c if it passes through all the previous if statements.
